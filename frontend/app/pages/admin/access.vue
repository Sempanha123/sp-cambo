  ...
  >
    ...
    <div class="space-y-6">
      ...
      <SpAsyncSection :loading="usage.initialLoading.value" :unavailable="usage.unavailable.value" :failed="usage.failed.value" :error-message="usage.error.value?.message" error-title="Usage metering could not be loaded" @retry="usage.refresh()">
        <UCard class="sp-app-card">
          <template #header><div><p class="font-semibold text-highlighted">Usage & metering</p><p class="text-sm text-muted">Latest 100 request records with public/private route, reservation estimate and final metering.</p></div></template>
          <div class="overflow-x-auto"><table class="w-full min-w-[1250px] text-left text-sm"><thead class="text-xs uppercase text-muted"><tr><th class="pb-3">Request</th><th class="pb-3">Customer</th><th class="pb-3">State</th><th class="pb-3">Route</th><th class="pb-3">Input / output</th><th class="pb-3">Metered</th><th class="pb-3">Duration</th><th class="pb-3">Started</th></tr></thead><tbody class="divide-y divide-default"><tr v-for="row in filteredUsage" :key="row.request_id"><td class="py-3"><p class="max-w-40 truncate font-mono text-xs text-highlighted" :title="row.request_id">{{ row.request_id }}</p><p class="text-xs text-muted">{{ row.endpoint }}</p></td><td class="py-3"><p>{{ row.user?.name ?? '—' }}</p><p class="text-xs text-muted">{{ row.user?.email }}</p></td><td class="py-3"><UBadge :color="statusColor(row.state)" variant="subtle">{{ row.state }}</UBadge><p v-if="row.error_code" class="mt-1 text-xs" :class="row.state === 'RECONCILING' ? 'text-warning' : 'text-error'">{{ row.error_code }}</p></td><td class="py-3"><p class="font-medium">{{ row.public_model }}</p><p class="text-xs text-muted">{{ row.provider ?? '—' }} · {{ row.internal_model ?? '—' }} · v{{ row.route_version ?? '—' }}</p></td><td class="py-3">{{ number(row.input_tokens) }} / {{ number(row.output_tokens) }}</td><td class="py-3"><p>{{ number(row.metered_units ?? row.estimated_units) }}</p><p class="text-xs text-muted">{{ row.metered_units ? 'final' : 'reserved estimate' }}</p></td><td class="py-3">{{ row.duration_ms === null ? (['RESERVED', 'CONNECTING', 'STREAMING'].includes(row.state) ? 'running' : '—') : `${number(row.duration_ms)} ms` }}</td><td class="py-3 text-xs">{{ date(row.started_at) }}</td></tr></tbody></table></div>
        </UCard>
      </SpAsyncSection>
    </div>
  </SpDashboardPage>
