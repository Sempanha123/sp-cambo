...
onMounted(() => {
  // Never make route navigation wait on the expensive 30-day aggregation.
  // Load it only after the lightweight details request has completed and the
  // browser has had time to paint the key card.
  const schedule = () => {
  ...
  if (details.data.value) schedule()
  else {
    const stop = watch(details.data, (value) => {
      if (!value) return
      stop()
      schedule()
    })
...
const balanceSourceLabel = computed(() => {
  ...
  if (source === 'dedicated_and_legacy_entitlements') return 'Dedicated key balance'
  if (source === 'legacy_account_entitlements') return 'Legacy/shared account balance'
  return 'No spendable balance assigned'
})
...
<template>
  ...
  >
    ...
    <SpAsyncSection
      :loading="details.initialLoading.value"
      ...
      <div v-if="key && details.data.value" class="space-y-5">
          ...
          <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div class="space-y-2">
              ...
              <p class="text-xs text-muted">
                The key is a credential. New purchases can be dedicated to this key, while older legacy balances may remain account-wide. This key cannot spend Playground-only or another key's dedicated package.
              </p>
            </div>
            <div class="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
              <div class="rounded-lg border border-default px-3 py-2">
                <p class="text-xs text-muted">Purchased tokens</p>
                <strong class="sp-numeric">{{ formatUnits(tokenRemaining) }}</strong>
              </div>
              <div class="rounded-lg border border-default px-3 py-2">
                <p class="text-xs text-muted">Credit</p>
                <strong class="text-xs">{{ creditLabel }}</strong>
              </div>
              <div class="rounded-lg border border-default px-3 py-2">
        ...
        <div v-if="activeTab === 'overview'" class="grid gap-4 md:grid-cols-2">
          ...
          <UCard class="sp-app-card">
            ...
            <p v-if="fundingLots.length === 0" class="text-sm text-muted">
          ...
          <UCard class="md:col-span-2 sp-app-card">
            ...
            <div v-if="usage.data.value" class="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              ...
              <div class="rounded-lg border border-default p-3"><p class="text-xs text-muted">Input tokens</p><strong class="sp-numeric">{{ usage.data.value.input_tokens.toLocaleString() }}</strong></div>
              <div class="rounded-lg border border-default p-3"><p class="text-xs text-muted">Output tokens</p><strong class="sp-numeric">{{ usage.data.value.output_tokens.toLocaleString() }}</strong></div>
              <div class="rounded-lg border border-default p-3"><p class="text-xs text-muted">Metered units</p><strong class="sp-numeric">{{ formatUnits(usage.data.value.metered_units) }}</strong></div>
              <div class="rounded-lg border border-default p-3"><p class="text-xs text-muted">Credit charge</p><strong>{{ formatMoney(usage.data.value.credit_charge) }}</strong></div>
            </div>
        ...
        <div v-else-if="activeTab === 'models'" class="space-y-4">
          ...
          <div v-if="key.allowed_model_aliases.length" class="grid gap-3 lg:grid-cols-2">
            <UCard v-for="alias in key.allowed_model_aliases" :key="alias" class="sp-app-card">
              <template #header>
                <div class="flex items-center justify-between gap-3">
                  <strong class="font-mono">{{ alias }}</strong>
        ...
        <div v-else-if="activeTab === 'activity'" class="space-y-3">
          ...
          <div v-if="(usage.data.value?.by_model ?? []).length" class="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            <div v-for="model in usage.data.value?.by_model ?? []" :key="model.public_model" class="rounded-lg border border-default bg-elevated/30 p-3 text-sm">
              <div class="flex items-center justify-between gap-2"><strong class="font-mono">{{ model.public_model }}</strong><span class="sp-numeric text-muted">{{ model.requests }} req</span></div>
              <p class="mt-1 text-xs text-muted">{{ formatUnits(model.metered_units) }} metered units · {{ formatMoney(model.credit_charge) }}</p>
            </div>
          </div>
            ...
            <table class="w-full text-sm">
              ...
              <tbody>
                <tr v-for="row in activity.data.value ?? []" :key="row.id" class="border-b border-default/60 last:border-0">
                  ...
                  <td class="p-3 text-right sp-numeric">{{ row.total_tokens?.toLocaleString() ?? '—' }}</td>
                  <td class="p-3 text-right sp-numeric">{{ row.duration_ms === null ? '—' : `${row.duration_ms.toLocaleString()} ms` }}</td>
                </tr>
              </tbody>
            </table>
          </div>
