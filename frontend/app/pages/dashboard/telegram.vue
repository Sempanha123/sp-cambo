...
<template>
  <SpDashboardPage
    ...
    description="SP Cambo has its own Telegram storefront: browse packages, pay with Bakong KHQR, receive an API key, check orders/balance, and control product updates without a website login."
  >
    <div class="grid gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(20rem,.85fr)]">
      <UCard class="sp-premium-card sp-app-card">
        <template #header>
          <div>
            <h2 class="font-semibold text-highlighted">
              ...
              A private Telegram identity gets its own SP Cambo customer workspace automatically. The old /link command remains compatibility-only.
            </p>
          </div>
        </template>
        <div class="grid gap-3 sm:grid-cols-2">
          ...
          >
            <strong class="text-sm text-highlighted">{{ feature[0] }}</strong><p class="mt-1 text-xs leading-5 text-muted">
              {{ feature[1] }}
            </p>
          </div>
        </div>
        ...
        <UAlert
          ...
          description="Set NUXT_PUBLIC_TELEGRAM_BOT_USERNAME to your bot username (without @). Laravel still uses TELEGRAM_BOT_TOKEN and TELEGRAM_WEBHOOK_SECRET for the real bot connection."
        />
      </UCard>
      ...
      <div class="space-y-5">
        ...
        <UAlert
          ...
          title="Automatic store updates"
          description="When an admin publishes a new model or a sellable package changes, SP Cambo queues an opt-in Telegram announcement. Package messages can include a Buy button. Customers can mute updates at any time."
        />
        <UAlert
          color="neutral"
          variant="subtle"
          icon="i-lucide-refresh-cw"
          title="Keep the scheduler running"
          description="telegram:reconcile-purchases verifies pending orders; telegram:broadcast-announcements delivers queued model/package updates. Both run every minute in the Laravel scheduler."
        />
      </div>
    </div>
  </SpDashboardPage>
