...
try {
    Run-Step 'Vue resolution' {
        node -e "const p=require.resolve('vue/package.json'); console.log(p); if(!p.replace(/\//g,'\\').toLowerCase().includes('\\frontend\\')) process.exit(2)"
    }
...
Write-Host ''
Write-Host '[PASS] SP Cambo Vue Runtime Cleanup R8.2 verification completed.'
