...
class SellCatalogSeederTest extends TestCase
{
    ...
    public function test_r27_seed_builds_three_stable_combos_nine_public_aliases_and_competitive_packages(): void
    {
        ...
        $this->assertSame(
            $expectedAliases,
            ModelAlias::query()->where('enabled', true)->where('customer_visible', true)->orderBy('public_alias')->pluck('public_alias')->all(),
        );

        $this->assertSame('AgentRouter-claude-opus-5', ModelAlias::query()->where('public_alias', 'opus-5')->firstOrFail()->model()->value('internal_model_id'));
        $this->assertSame('OpenAI Codex', ModelAlias::query()->where('public_alias', '5.6-sol')->firstOrFail()->model()->value('internal_model_id'));
        $this->assertSame('OpenAI Codex', ModelAlias::query()->where('public_alias', '4.8-sol')->firstOrFail()->model()->value('internal_model_id'));
        $this->assertSame('Gemini Google AI Studio', ModelAlias::query()->where('public_alias', 'gemini-3.6-flash')->firstOrFail()->model()->value('internal_model_id'));
        ...
        $this->assertSame(229, (int) $claude100->price_minor);
        $this->assertSame(100_000_000, (int) $claude100->advertised_units);
        $this->assertSame(86400, (int) $claude100->duration_seconds);
        $this->assertSame(['haiku-4.5', 'opus-5', 'sonnet-5'], $claude100->modelAliases()->orderBy('public_alias')->pluck('public_alias')->all());

        $codex50 = Package::query()->where('slug', 'codex-token-50m')->firstOrFail();
        $this->assertSame(349, (int) $codex50->price_minor);
        $this->assertSame(['4.8-sol', '5.6-sol', 'openai-codex'], $codex50->modelAliases()->orderBy('public_alias')->pluck('public_alias')->all());

        $gemini100 = Package::query()->where('slug', 'gemini-token-100m')->firstOrFail();
        $this->assertSame(189, (int) $gemini100->price_minor);
        $this->assertSame(['gemini-3.6-flash', 'gemini-3.6-pro', 'gemini-google-ai-studio'], $gemini100->modelAliases()->orderBy('public_alias')->pluck('public_alias')->all());
        ...
        $this->assertSame($expectedAliases, collect($setting->allowed_model_aliases)->sort()->values()->all());
        ...
        $this->assertNull(Provider::query()->where('slug', 'omniroute-primary')->value('active_connection_revision_id'));
    }
    ...
    public function test_reseed_preserves_public_to_private_combo_mapping(): void
    {
        ...
        ModelAlias::query()->where('public_alias', '5.6-sol')->update(['display_name' => 'Temporary label']);
        ModelAlias::query()->where('public_alias', 'opus-5')->update(['display_name' => 'Temporary Claude label']);

        $this->seed(SellCatalogSeeder::class);

        $this->assertSame('GPT-5.6 Sol', ModelAlias::query()->where('public_alias', '5.6-sol')->value('display_name'));
        $this->assertSame('Claude Opus 5', ModelAlias::query()->where('public_alias', 'opus-5')->value('display_name'));
        $this->assertSame('OpenAI Codex', ModelAlias::query()->where('public_alias', '5.6-sol')->firstOrFail()->model()->value('internal_model_id'));
        $this->assertSame('AgentRouter-claude-opus-5', ModelAlias::query()->where('public_alias', 'opus-5')->firstOrFail()->model()->value('internal_model_id'));
    }
}
