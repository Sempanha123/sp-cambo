...
class ResellerCustomerController extends Controller
{
    ...
    {
        $customers = ResellerCustomer::query()
            ->where('reseller_user_id', $request->user()->id)
            ->join('users', 'users.id', '=', 'reseller_customers.customer_user_id')
            ->select('reseller_customers.*', 'users.name', 'users.email')
            ->orderBy('reseller_customers.id')
            ->get();

        return response()->json([
            'data' => $customers->map(fn ($customer) => $this->resource($customer)),
        ]);
    }
    ...
    public function store(Request $request, AuditService $audit): JsonResponse
    {
        ...
        $managed = DB::transaction(function () use ($request, $data, $audit): ResellerCustomer {
            ...
            $managed = ResellerCustomer::query()->create([
                'reseller_user_id' => $request->user()->id,
            ...
            $audit->record(
                $request->user(),
                'reseller_customer.created',
                'user',
                $customer->id,
                'Reseller created managed customer.',
                ['reseller_customer_id' => $managed->id]
            );
    ...
    public function updateStatus(
        ...
        $managed = DB::transaction(function () use (
            $request,
            $resellerCustomer,
            $data,
            $audit
            ...
            $customer = User::query()
                ->lockForUpdate()
                ->findOrFail($managed->customer_user_id);
    ...
    /**
     * One endpoint supports both formats:
     *
     * Preferred package sale:
     * {
     *   "inventory_lot_id": "...",
     *   "units": 1000000,
     *   "idempotency_key": "...",
     ...
     * }
     *
     * Legacy single-model allocation remains supported.
     */
    public function allocate(
        Request $request,
        string $resellerCustomer,
        ResellerAllocationService $allocations
    ): JsonResponse {
        $managed = ResellerCustomer::query()
            ->where('reseller_user_id', $request->user()->id)
            ->where('status', 'ACTIVE')
        ...
        $customer = User::query()->findOrFail($managed->customer_user_id);

        if ($request->has('inventory_lot_id')) {
            $data = $request->validate([
                'inventory_lot_id' => ['required', 'string', 'max:64'],
                'units' => ['required', 'integer', 'min:1'],
                'idempotency_key' => ['required', 'string', 'max:191'],
                'reason' => ['required', 'string', 'min:10', 'max:2000'],
            ]);

            $transfer = $allocations->allocatePackage(
                $request->user(),
                $customer,
            ...
            $source = EntitlementLot::query()
                ...
                ->findOrFail($data['inventory_lot_id']);

            return response()->json([
                'data' => [
                    'id' => $transfer->id,
                    'customer_id' => (string) $managed->id,
                    'allocation_kind' => 'PACKAGE',
                    'inventory_lot_id' => (string) $source->id,
                    'package_name' => (string) $source->package_name,
                    'billing_mode' => (string) $source->billing_mode,
                    'public_model_alias' => null,
                    'allowed_model_aliases' => $source->allowed_model_aliases ?? [],
                    'units' => (string) $transfer->units,
                    'expires_at' => $source->expires_at?->toAtomString(),
        ...
        $data = $request->validate([
            'billing_mode' => ['required', 'in:TOKEN_QUOTA,CREDIT_BALANCE'],
            'public_model_alias' => ['required', 'string', 'max:100'],
            'units' => ['required', 'integer', 'min:1'],
            'idempotency_key' => ['required', 'string', 'max:191'],
            'reason' => ['required', 'string', 'min:10', 'max:2000'],
        ]);

        $transfer = $allocations->allocate(
            $request->user(),
            $customer,
            $data['billing_mode'],
            $data['public_model_alias'],
            (int) $data['units'],
            $data['idempotency_key'],
            $data['reason']
        );
