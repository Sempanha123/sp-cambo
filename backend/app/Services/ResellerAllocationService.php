    ...
    public function allocate(
        ...
        if ($units <= 0 || mb_strlen(trim($reason)) < 10) {
            throw ValidationException::withMessages([
                'allocation' => ['Allocation units and audit reason are invalid.'],
            ]);
        }
        ...
        /*
         * A seller can call allocation directly without first opening /inventory.
         * Adopt any historical untouched paid purchases before calculating stock.
         */
        ...
        return DB::transaction(function () use (
            ...
            $available = $lots->sum(
                fn (EntitlementLot $lot): int =>
                    max(0, $lot->remaining_units - $lot->reserved_units)
            );
            ...
            foreach ($lots as $source) {
                $take = min(
                    $needed,
                    max(0, $source->remaining_units - $source->reserved_units)
                );
