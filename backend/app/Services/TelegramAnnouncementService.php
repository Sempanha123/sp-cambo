...
", array_filter([
                ...
                'Tap Buy now to shop eligible packages.',
            ])),
            'package_id' => $package?->id,
        ]);
    }
    ...
    private function message(TelegramAnnouncement $announcement, TelegramAccount $account): array
    {
        ...
        $heading = match ($announcement->kind) {
            ...
            'STOCK_ADDED' => $km ? 'បានបន្ថែមស្តុក' : 'Stock added',
            'RESTOCK' => $km ? 'មានស្តុកវិញហើយ' : 'Back in stock',
            'PROMOTION' => $km ? 'ប្រូម៉ូសិនថ្មី' : 'Special offer',
            'PURCHASE_ACTIVITY' => $km ? 'ការបញ្ជាទិញថ្មី!' : 'NEW ORDER!',
    ...
    private function purchaseActivityBody(TelegramAnnouncement $announcement, bool $km): string
    {
        ...
        $template = $configured !== ''
            ...
            : ($km
                ...
                : "👤 {masked_customer} just purchased\n📦 {package_name}\n🪙 {quota}\n⏱ {validity}\n💰 {price} {currency}\n✅ Successfully delivered\n\n🛒 Get yours below");
