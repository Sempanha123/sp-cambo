...
export function useSpApi() {
  ...
  const streamPlayground = async (
    ...
    try {
      ...
      if (playgroundDoneSeen) {
        try {
          await reader.cancel('sp_playground_done')
        } catch {
          // The server may already have closed the completed stream.
        }
      }
      reader.releaseLock()
    }
