...
try {
    Run-Step 'Vue resolution' {
        node -e "const p=require.resolve('vue/package.json'); console.log(p); const normalized=p.replace(/\//g,'\\'); if(!normalized.toLowerCase().includes('\\frontend\\')) process.exit(2)"
    }
...
Write-Host ''
Write-Host '[PASS] SP Cambo Vue Runtime Cleanup R8.1 verification completed.'
