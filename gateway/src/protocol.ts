...
export const LOCAL_OUTPUT_CALIBRATION_BPS = 15_500;
...
export function upstreamBody(path: InferencePath, prepared: Prepared, internalModel: string, hardMax: number): string {
  ...
  // Always make stream mode explicit to the private upstream.
  // Prevent OmniRoute from choosing its own stream default when omitted.
  if (path !== "/v1/messages/count_tokens") {
    body.stream = prepared.streaming;
  }
