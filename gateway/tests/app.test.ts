...
it("maps public model, strips customer auth and settles SP-local JSON usage", async () => {
  ...
  expect(response.statusCode).toBe(200); expect(response.json().model).toBe("claude-coding"); expect(response.body).not.toContain("private-route"); expect(control.settleCalls[0]?.usage).toMatchObject({ input_tokens: estimateTokens(JSON.stringify(body)), output_tokens: 4, cache_read_tokens: 0, cache_write_tokens: 0, reasoning_tokens: 0 }); expect(control.releases).toHaveLength(0);
});
...
it("restores exact Claude Code tool casing in streamed Anthropic responses", async () => {
  ...
  const stream = [
    `event: content_block_start\ndata: ${JSON.stringify({ type: "content_block_start", index: 0, content_block: { type: "tool_use", id: "tool_1", name: "edit", input: {} } })}\n\n`,
  ...
  expect(control.settleCalls[0]?.usage).toMatchObject({ input_tokens: estimateTokens(JSON.stringify(requestBody)), output_tokens: 4 });
});
